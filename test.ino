...
unsigned long lastHeightAdjustTime = 0;
const unsigned long sprintDuration = 3000000; // 3 seconds
unsigned long lastFoundTime = 0;
const unsigned long FoundDuration = 3000000; // 3 seconds
...
float z_estimator = 0;
bool charge = false;
float total_rotate = 0.0;
float max_rotate = 2* M_PI;
float last_detect_yaw = 0.0;
float last_detect_height = 0.0;
bool found = false;
...
void loop() {
  ...
  if (cmd.params[0] == 2) {
    ...
    //Charge
    if(charge){
      //Exceed Charge session
      if (micros() - lastHeightAdjustTime > sprintDuration) {
        charge = false;
        return;
      }
      else{
        behave.params[1] = terms.fx_charge;
      ...
      //Search
        ...
        //Rotate
        if(found){
          if(micros()- lastFoundTime <= FoundDuration){
              behave.params[1] = 0;
              behave.params[0] = cmd.params[0]; // flag
              behave.params[2] = z_estimator; // fz (meters)
              behave.params[3] = 0; // tx (radians/second)
          ...
          if(total_rotate <= max_rotate){
            float _yaw = senses[5];  
            float offset_yaw = 0.2;
            total_rotate += offset_yaw;
            //nicla_yaw = _yaw + offset_yaw;
            behave.params[0] = cmd.params[0]; // flag
            behave.params[1] = cmd.params[1]; // fx ('meters'/second)
            behave.params[2] = z_estimator; // fz (meters)
            behave.params[3] = 0; // tx (radians/second)
            behave.params[4] =  _yaw + offset_yaw; // tz (radians)
          }
          else{
            charge = true;
            total_rotate = 0.0;
