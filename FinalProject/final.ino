...
float z_estimator = 0;
bool charge = false;
float spiralRadius = terms.fx_charge;
//float spiralAngle = 0;
const float spiralIncrement = terms.fx_togoal;  
const float radiusIncrement = terms.fx_levy;  
...
void loop() {
  ...
  if (cmd.params[0] == 2) {
    ...
    if (nicla_flag != 0) {// checks if new yaw occurs
      ...
      //float spiral = cmd.params[1] + radiusIncrement;
      float _yaw = senses[5];
      nicla_yaw = _yaw + spiralIncrement;  
      //nicla_yaw = fmod(nicla_yaw + spiralIncrement, 2 * M_PI); 
      behave.params[0] = cmd.params[0]; // flag
      behave.params[1] = cmd.params[1]; // fx ('meters'/second)
      behave.params[2] = z_estimator; // fz (meters)
      behave.params[3] = 0; // tx (radians/second)
      behave.params[4] = _yaw + spiralIncrement; // tz (radians)
    } 
